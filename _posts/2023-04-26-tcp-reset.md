...
<figure>
  ...
  <figcaption>The existing telnet connection with a command run</figcaption>
</figure>
...
In the next part, we will expand on the knowledge gained from conducting a RESET attack to hijack the `telnet` connection to run an arbitrary command.